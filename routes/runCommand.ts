...
export const runCommandHandler = (req: Request, res: Response) => {
  ...
  if (!fs.existsSync(envParsedWithTypes.USER_PROJECT_CONTAINER_LOCATION)) {
  ...
  exec(command, { cwd: envParsedWithTypes.USER_PROJECT_CONTAINER_LOCATION }, (err, stdout, stderr) => {
    if (stderr) {
    ...
    exec('git diff', { cwd: envParsedWithTypes.USER_PROJECT_CONTAINER_LOCATION }, (diffErr, diffStdout, diffStderr) => {
      if (diffStderr) {
